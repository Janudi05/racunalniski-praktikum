...
% Vsi potrebni paketi iz naloge 1
...
\begin{center}
   \item   \(velikostTabele - steviloUrNaDan \cdot steviloDelovnihDni\)
   \end{center}
...
spremembi izmeri njegovo kakovost. Tako moramo na vsakem koraku ugotavljati, katerega
omejitve so bile kršene ter kolikokrat se kršitev pojavi. S \emph{kn} označimo število
kršitev omejitve \emph{n}, z \emph{un}  pa njeno utež. Z naslednjo formulo izračunamo
kakovost urnika:

\begin{center}
   \item   \(k1 \cdot u1 + k2 \cdot u2 + . . . + kn-1 \cdot un-1 + kn \cdot un\)
   \end{center} 

\noindent Nižja vrednost formule pomeni kakovostnejši urnik, izračunamo pa jo na vsakem koraku.
...
\begin{enumerate}
   \item Z zanko se sprehodimo po vektorju vseh predmetov. Predmet na \emph{i}-tem koraku
označimo s predmet\emph{i}.
   \item Naslednja vgnezdena zanka nas popelje po vseh predavalnicah. Predavalnico na
\emph{j}-tem koraku označimo s predavalnica\emph{j}
\item Za vse proste termine predavalnice\emph{j} izračunamo ceno vstavljanja srečanja\emph{i}, ki ga sestavlja predmet\emph{i}, profesor, ki ta predmet predava,
...
vstavljanja, da bomo predmet\emph{i} na koncu vstavili v termin z najboljšo
ceno vstavljanja.
   \item Ko preverimo vse proste termine vseh predavalnic, vstavimo srečanje\emph{i}
...
Rešitev se ponuja kar sama - za naslednjih \emph{n} korakov prepovemo vsako spremembo, ki jo
opravimo v koraku optimizacije. Izbira konstante \emph{n} je odvisna od samega problema, saj
ne obstaja neki splošen \emph{n}, ki bi nas v vsakem primeru pripeljal iz lokalnega ekstrema.
...
Naj bo ?\emph{n} = 100 ter \emph{s1} sprememba urnika, ki nas je pripeljala v lokalni maksimum. To
spremembo (npr. zamenjava srečanja\emph{i} in \emph{j}) sedaj shranimo v
seznam tabujev. V naslednjem koraku izvedemo spremembo \emph{s2}, ki se prav tako shrani v
seznam prepovedanih sprememb. Podobno naredimo za vse spremembe v nadaljnih korakih. S
tem poskrbimo, da se lahko v isti lokalni maksimum vrnemo šele čez \emph{n} = 100 korakov, saj
se sprememba \emph{s1}, ki vodi do lokalnega maksimuma, šele tedaj izbriše iz seznama
prepovedanih sprememb.

Če smo dosegli lokalni maksimum in vrednost kriterijske funkcije v \emph{n} korakih uspemo
...
\begin{enumerate}
   ...
   \item Rešitev \emph{r}, ki smo jo dobili s pomočjo požrešne metode, izboljšamo z lokalnim vzpenjanjem
   ...
   \item Na vsakem koraku ocenimo izboljšamo rešitev \emph{r}. Če je vrednost kriterijske funkcije
   za trenutno rešitev prešla zastavljeno zadovoljivo mejo, potem se program zaključi. Prav tako
   se zaključi, če po \emph{n} korakih še vedno tičimo v istem lokalnem maksimumu.
\end{enumerate}
% Spodnja vrstica ne začne novega odstavka, zato ne bi smela biti zamaknjena.
% To popravite z ukazom `\noindent` ali pa tako, da odstranite prazno vrstico.
% (Nekoliko lepše izgleda, če v prazno vrstico vstavite % za komentar.)
